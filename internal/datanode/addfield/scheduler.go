...
func (s *scheduler) Start() {
	log.Info("start schema change scheduler")
	var (
	...
	for {
		select {
		case <-s.closeChan:
			log.Info("schema change scheduler exited")
			return
		case <-exeTicker.C:
			tasks := s.manager.GetBy(WithStatesOfSCTask(internalpb.SchemaChangeState_pending))
			...
			for taskID, fs := range futures {
				...
				s.manager.Update(taskID, UpdateStateOfSCTask(internalpb.SchemaChangeState_completed))
				log.Info("schema change done", zap.Int64("taskID", taskID))
			}
		case <-logTicker.C:
			// log here
		}
...
func (s *scheduler) Slots() int64 {
	tasks := s.manager.GetBy(WithStatesOfSCTask(internalpb.SchemaChangeState_pending, internalpb.SchemaChangeState_progressing))
	used := lo.SumBy(tasks, func(t Task) int64 {
		return t.GetSlots()
	})
	// todo: remove it to yam;
	total := int64(10)
	free := total - used
	if free >= 0 {
		return free
