...
type TaskManager interface {
	Add(task Task)
	Update(taskID int64, actions ...UpdateActionOfSCTask)
	Get(taskID int64) Task
	GetBy(filters ...FilterOfSCTask) []Task
	Remove(taskID int64)
}
...
func (m *taskManager) Update(taskID int64, actions ...UpdateActionOfSCTask) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *taskManager) GetBy(filters ...FilterOfSCTask) []Task {
	m.mu.RLock()
	defer m.mu.RUnlock()
	ret := make([]Task, 0)
OUTER:
